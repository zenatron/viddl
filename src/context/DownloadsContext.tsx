...
export interface DownloadItem {
  ...
  status: 'starting' | 'downloading' | 'complete' | 'error' | 'cancelled' | 'pending'; // Added 'pending' for items added before init response
  eta?: string;
  error?: string;
}
...
interface DownloadsContextType {
  ...
  addDownload: (item: Omit<DownloadItem, 'id' | 'status' | 'progress' | 'speed' | 'total_bytes' | 'downloaded_bytes'>) => Promise<void>; // Takes basic info, gets ID from backend
  updateDownloadProgress: (id: string, progressData: Partial<DownloadItem>) => void;
...
export const DownloadsProvider: React.FC<DownloadsProviderProps> = ({ children }) => {
  ...
  const addDownload = useCallback(async (itemInfo: Omit<DownloadItem, 'id' | 'status' | 'progress' | 'speed' | 'total_bytes' | 'downloaded_bytes'>) => {
    ...
    try {
      ...
      const initResponse = await fetch(`/api/download?url=${encodeURIComponent(itemInfo.url)}&quality=${itemInfo.quality}`, {
        headers: {
          'X-Request-Type': 'init'
        }
      });
      ...
      if (!initResponse.ok) {
        ...
        throw new Error(errorData.error || `Download initialization failed with status: ${initResponse.status}`);
      }
      ...
      setDownloads(prev => 
          prev.map(d => d.id === tempId ? { ...d, id: downloadId, status: 'starting' } : d)
      );
  ...
  const updateDownloadProgress = useCallback((id: string, progressData: Partial<DownloadItem>) => {
    setDownloads(prev =>
  ...
  const cancelDownload = useCallback(async (id: string) => {
    ...
    if (!download || download.status === 'pending' || download.status === 'complete' || download.status === 'error' || download.status === 'cancelled') {
      console.log(`Download ${id} not cancellable (status: ${download?.status})`);
      return;
    }
    
    // Optimistically update UI
    updateDownloadProgress(id, { status: 'cancelled', error: 'Cancelling...' }); 
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Cancellation failed with status: ${response.status}`);
      }
  ...
  return (
    <DownloadsContext.Provider value={{ downloads, addDownload, updateDownloadProgress, removeDownload, cancelDownload }}>
      {children}
