...
const mapStatusForProgressBar = (status: DownloadItem['status']): ProgressBarProps['status'] => {
  switch (status) {
    case 'downloading':
      return 'downloading';
    case 'complete':
...
export function ActiveDownloadsList() {
  const { downloads, updateDownloadProgress, removeDownload, cancelDownload } = useDownloads();
  ...
  const poll = useCallback(async () => {
    ...
    const activeDownloads = currentDownloads.filter(d => 
    ...
    console.log(`[ActiveDownloadsList] Poll function CALLED for IDs: ${activeIds.join(', ')}`);
    
    try {
      const response = await fetch(`/api/download?progressId=${activeIds.join(',')}`);
      
      if (!response.ok) {
        console.error(`Polling failed for batch ${activeIds.join(', ')}: ${response.status}`);
        activeIds.forEach(id => updateDownloadProgress(id, { status: 'error', error: `Polling failed (${response.status})` }));
        return;
      }
      ...
      for (const [id, data] of Object.entries(batchData)) {
         if (data) {
             ...
             if (data.status === 'error' && data.error?.includes('not found')) {
                 console.log(`Progress for ${id} not found within batch, likely expired or cancelled.`);
      ...
      console.error(`Error during batch polling for ${activeIds.join(', ')}:`, error);
      activeIds.forEach(id => updateDownloadProgress(id, {
          status: 'error', 
  ...
  useEffect(() => {
    ...
    const startPolling = () => {
        if (!pollingIntervalRef.current && downloadsRef.current.some(d => (d.status === 'starting' || d.status === 'downloading') && !d.id.startsWith('temp-'))) {
  ...
  useEffect(() => {
      const hasActiveDownloads = downloads.some(d => (d.status === 'starting' || d.status === 'downloading') && !d.id.startsWith('temp-'));
      if (!hasActiveDownloads && pollingIntervalRef.current) {
          console.log('Stopping polling interval - no active downloads found in state update.');
          clearInterval(pollingIntervalRef.current);
          pollingIntervalRef.current = null;
      } else if (hasActiveDownloads && !pollingIntervalRef.current) {
           // If polling stopped but active downloads appeared again, restart it
           console.log('Restarting polling interval - active downloads found in state update.');
            poll(); // Poll immediately
            pollingIntervalRef.current = setInterval(poll, 1000); 
      }
  ...
  return (
    <div className="w-full mt-6 space-y-3">
      <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200">Downloads</h2>
      {downloads.map((download) => (
        <div key={download.id} className="p-3 border rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700 shadow-sm">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-medium text-gray-800 dark:text-gray-100 truncate mr-2" title={download.filename}>
            ...
            {(download.status === 'complete' || download.status === 'error' || download.status === 'cancelled') && (
               <button
