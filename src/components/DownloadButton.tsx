      ...
      const fetchProgress = async () => {
        try {
          ...
          const response = await fetch(
            `/api/download?progressId=${downloadId}`,
          );
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const initResponse = await fetch(
        `/api/download?url=${encodeURIComponent(videoInfo.directDownloadUrl)}&quality=${selectedQuality}`,
        {
          headers: {
            "X-Request-Type": "init",
          },
      ...
      if (!downloadId && newDownloadId) {
        // If polling hasn't started but we got an ID
        // Optionally, tell the server to clean up (needs backend support)
      } else if (!downloadId) {
        // If we didn't even get an ID
